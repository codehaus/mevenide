 ...
 * 
 */
public abstract class AbstractNbMevenideTag extends TagSupport {
    ...
    protected void checkAttribute(String attribute, String attributeName, String[] allowedValues) throws MissingAttributeException {
        checkAttribute((Object) attribute, attributeName);
        if (attribute.trim().equals("") ) {
            throw new MissingAttributeException(attributeName + " should not be empty.");
        }
        if (allowedValues != null && allowedValues.length > 0) {
            ...
            for (int i = 0; i < allowedValues.length; i++) {
                if (attribute.equals(allowedValues[i])) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                throw new MissingAttributeException(attributeName + " contains not allowed value \"" + attribute + "\"");
            }
