...
//            System.out.println("test artifact=" + node);
        ...
        public void omitForNearer(Artifact omitted, Artifact kept) {
            if (omitted.equals(kept)) {
                System.out.println("omiting the same=" + kept);
                ArtifactGraphNode nd = getNode(omitted);
                nd.setArtifact(kept);
                return;
            }
...
//            } 
        ...
        public void manageArtifact(Artifact artifact, Artifact replacement) {
            if (artifact.equals(replacement)) {
                System.out.println("managing the same=" + replacement);
                ArtifactGraphNode nd = getNode(artifact);
                nd.setArtifact(replacement);
                return;
            }
            System.out.println("managing new =" + replacement);
            System.out.println("         old =" + artifact);
            ...
            for (ArtifactGraphEdge edge : scene.getEdges()) {
                if (getNode(artifact) == scene.getEdgeSource(edge)) {
                    System.out.println("    replacing source=" + edge.toString());
                    scene.setEdgeSource(edge, getNode(replacement));
                }
                if (getNode(artifact) == scene.getEdgeTarget(edge)) {
                    System.out.println("    replacing target=" + edge.toString());
