...
public class ModuleSettingsPanel extends JPanel {
    ...
    /**
     * The global maven goals tree. We need a reference to it to retrieve the current selection when
    ...
    /**
     * The favorite goals list model - used when the "Add" or "Remove" buttons are pressed to
     * add/remove goals.
     */
    ...
    protected void init() {
        ...
        //
        final JButton addButton = new JButton(new ImageIcon(Images.PLAY));
        ...
        final JButton removeButton = new JButton(new ImageIcon(Images.PLAY_REVERSED));
    ...
    public void loadMavenGoals(final IGoalsGrabber pGoalsGrabber) {
        mavenGoalsTree.setModel(new MavenGoalsTreeModel(pGoalsGrabber));
    }
    ...
    protected void removeSelection() {
        ...
        for (int i = 0; i < selections.length; i++)
            data[i] = favoriteGoalsModel.get(i);
