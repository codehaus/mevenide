...
public class FilterAction extends AbstractBaseAction {
    ...
    if (actionText.equals(PluginConstants.ACTION_COMMAND_FILTER) ||
      ...
      try {
        if (!actionContext.getProjectPluginSettings().isUseFilter()) {
          ...
          actionEvent.getPresentation()
              .setIcon(GuiUtils.createImageIcon(PluginConstants.ICON_FILTER_APPLIED));
          ...
          actionEvent.getPresentation()
              .setIcon(GuiUtils.createImageIcon(PluginConstants.ICON_FILTER));
  ...
  private void toggleFilter(ActionContext context, boolean doFilter) {
    ...
    Enumeration expandedPaths = pomTree.getExpandedDescendants(
        new TreePath(((DefaultMutableTreeNode) pomTree.getModel().getRoot()).getPath()));
    if (doFilter) {
      ActionUtils.filterStandardPhasesInNodes(context,
          (DefaultMutableTreeNode) pomTree.getModel().getRoot());
    } else {
      ActionUtils.unfilterStandardPhasesInNodes(context,
          (DefaultMutableTreeNode) pomTree.getModel().getRoot());
    }
