...
package de.qu.idea.plugin.maven.build;

import com.intellij.openapi.progress.ProgressIndicator;
import com.intellij.openapi.progress.ProgressManager;
import de.qu.idea.plugin.maven.build.util.BuildConstants;
import de.qu.idea.plugin.maven.util.CommonUtils;
...
public class MavenBuildTask extends AbstractMavenBuildTask implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      mavenProcess =
          rt.exec(executionCommand.toArray(cmdArray), null,
      ...
      logger.info(command + System.getProperty("line.separator") +
          System.getProperty("line.separator"), LogListener.OUTPUT_TYPE_SYSTEM);
      logger.setOutputType(LogListener.OUTPUT_TYPE_NORMAL);
      BufferedReader procout =
          new BufferedReader(new InputStreamReader(mavenProcess.getInputStream()));
      String line;
      ...
      if (isCancelled()) {
        logger.info(
            System.getProperty("line.separator") + BuildConstants
                .MESSAGE_PROCESS_ABORTED +
                (mavenProcess != null ? mavenProcess.exitValue() : "-1"),
            LogListener.OUTPUT_TYPE_SYSTEM);
      } else {
        if (exitValue == 0) {
          logger.info(
              System.getProperty("line.separator") + BuildConstants.MESSAGE_PROCESS_FINISHED +
                  mavenProcess.exitValue(), LogListener.OUTPUT_TYPE_SYSTEM);
          // Terminated by user
        } else {
          logger.error(
              System.getProperty("line.separator") + BuildConstants
                  .MESSAGE_PROCESS_ABNORMALLY_TERMINATED +
                  (mavenProcess != null ? mavenProcess.exitValue() : "-1"),
              LogListener.OUTPUT_TYPE_SYSTEM);
        }
      }
      ...
      int exitValue = stop();
      logger.info(System.getProperty("line.separator") + BuildConstants
          .MESSAGE_PROCESS_ABORTED + exitValue, LogListener.OUTPUT_TYPE_SYSTEM);
      // catch MavenBuildExceptions
    } catch (MavenBuildException ex) {
      int exitValue = stop();
      logger.info(System.getProperty("line.separator") + BuildConstants
          .MESSAGE_PROCESS_ABNORMALLY_TERMINATED + exitValue, LogListener.OUTPUT_TYPE_SYSTEM);
