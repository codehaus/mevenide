...
public class BuildHelper {
  ...
  private static boolean isDefaultSettingsFile(String settingsFile) {
    return settingsFile
        .equals(System.getProperty("user.home") + System.getProperty("file.separator") + ".m2"
            + System.getProperty("file.separator") + BuildConstants
            .FILENAME_MAVEN_SETTINGS_FILE);
  }

  public static List<String> createMavenExecutionCommand(IBuildEnvironment buildEnvironment) throws
      MavenBuildException {
    String mavenHome;
    ...
    if (SystemUtils.IS_OS_WINDOWS || SystemUtils.IS_OS_WINDOWS_2000 || SystemUtils.IS_OS_WINDOWS_95
        ...
        || SystemUtils.IS_OS_WINDOWS_NT || SystemUtils.IS_OS_WINDOWS_XP) {
      classpathSeparator = ";";
    ...
    if (!StringUtils
        .isBlank(buildEnvironment.getMavenBuildSettings().getMavenCommandLineParams())) {
    ...
    if (StringUtils
        .isNotEmpty(buildEnvironment.getMavenBuildSettings().getMavenOptions().toString())) {
      String[] mavenOptions;
      mavenOptions =
          StringUtils.split(buildEnvironment.getMavenBuildSettings().getMavenOptions().toString());
  ...
  private static String getMavenHome(String mavenHome) throws MavenBuildException {
    ...
    if (StringUtils.isBlank(mavenHome)) {
      ...
      if (!StringUtils.isBlank(mavenSystemHome)) {
        ...
        throw new MavenBuildException(
            "Unable to execute Maven-2. No Maven-2 home directory specified!" +
    ...
    if (!mavenConfFile.exists()) {
      throw new MavenBuildException(
          "Specified Maven-2 home directory " + mavenHome + " is not a" +
              System.getProperty("line.separator") +
              "Maven-2 installation directory!");
    }
    try {
